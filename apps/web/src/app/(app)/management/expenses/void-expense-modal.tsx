...
const fieldClass =
  "rounded-[5px] border border-[color:var(--border-muted)] bg-white px-3 py-2 text-sm text-[var(--text-primary)] shadow-[var(--shadow-whisper)] outline-none transition focus:border-[var(--brand-burgundy)] focus:ring-4 focus:ring-[var(--focus-ring)]";
...
function VoidExpenseForm({
  ...
  const [state, formAction] = useActionState(voidExpense, initialFormState);
  ...
  return (
    <form action={formAction} className="grid gap-4">
      ...
      <div className="grid gap-1.5">
        <label className={labelClass} htmlFor={`void-reason-${expense.id}`}>
          Reason
        </label>
        <textarea
          ...
          minLength={3}
          name="reason"
          placeholder="Why is this expense being voided? e.g. duplicate entry, wrong amount"
          required
          rows={3}
        />
        <p className="text-xs text-[var(--text-muted)]">
          Voided expenses stay on record but are excluded from profit/loss.
        </p>
      </div>
...
export function VoidExpenseButton({
  ...
  return (
    <>
      <button
        className="inline-flex h-8 items-center justify-center rounded-[5px] border border-[color:var(--border-strong)] bg-white px-3 text-xs font-semibold text-[var(--text-secondary)] shadow-[var(--shadow-whisper)] transition hover:border-[var(--negative)] hover:text-[var(--negative)]"
